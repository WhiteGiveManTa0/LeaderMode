...
typedef struct VCR_s
{
	...
	long		(*Hook_RegSetValueEx)(void *hKey, char const *lpValueName, unsigned long Reserved, unsigned long dwType, uint8 const *lpData, unsigned long cbData);
	long		(*Hook_RegQueryValueEx)(void *hKey, char const *lpValueName, unsigned long *lpReserved, unsigned long *lpType, uint8 *lpData, unsigned long *lpcbData);
	...
	int			(*GenericPlayback)( const char *pEventName, void *pOutData, int maxLen, bool bForceLenSame );
	...
	void		(*GenericValueVerify)( const char *pEventName, const void *pData, int maxLen );
...
extern "C" __declspec(dllimport) VCR_t *g_pVCR;
