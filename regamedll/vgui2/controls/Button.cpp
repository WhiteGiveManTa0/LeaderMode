...
// Purpose: Constructor
//-----------------------------------------------------------------------------
Button::Button(Panel *parent, const char *panelName, const char *text) : Label(parent, panelName, text)
{
	Init();
}
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
Button::Button(Panel *parent, const char *panelName, const wchar_t *wszText) : Label(parent, panelName, wszText)
{
	Init();
}
...
void Button::ApplySchemeSettings(IScheme *pScheme)
{
	...
	_defaultFgColor = GetFgColor();
	_defaultBgColor = GetBgColor();
...
const char *Button::GetDescription( void )
{
	...
	_snprintf(buf, sizeof(buf), "%s, string command, int default", BaseClass::GetDescription());
	return buf;
}
