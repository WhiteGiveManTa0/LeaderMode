...
public:
	...
	int AddItem(const wchar_t *itemText, KeyValues *userData)
	{
		...
		if( text )
		{
			...
			text[ wcslen( itemText ) ] = '\0';
	
			// when the menu item is selected it will send the custom message "SetText"
			return m_pDropDown->AddMenuItem(text, new KeyValues("SetText", "text", text), this, userData);
		}
	...
	void OnMenuItemHighlight(int menuID)
	{
		char wbuf[80];
		if ( m_pDropDown->IsValidMenuID(menuID) )
		{
			m_pDropDown->GetMenuItem(menuID)->GetText(wbuf, 80);
		}
		else
		{
			wbuf[0] = 0;
...
void FileOpenDialog::ValidatePath()
{
	...
	HANDLE findHandle = ::FindFirstFileA(pData, &findData);
	if (findHandle != INVALID_HANDLE_VALUE)
	{
		...
		strncpy(m_szLastPath, pData, sizeof(m_szLastPath));
	}
	else
	{
		// failed to load file, use the previously successful path
		strcpy(pData, m_szLastPath);
	}
...
void FileOpenDialog::PopulateFileList()
{
	...
	if (combokv)
	{
		const char *pszFilter = combokv->GetString("filter", "*");
		strcat(dir, pszFilter);
	}
	else
	{
...
void FileOpenDialog::OnOpen()
{
	...
	WIN32_FIND_DATAA findData;
	HANDLE findHandle = ::FindFirstFileA(fullpath, &findData);
	if (findHandle != INVALID_HANDLE_VALUE)
	{
		// file found
		if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
	...
	else
	{
		...
		if (m_bOpenOnly &&  strlen(filename) > 0 )
		{
			// open the file!
			PostActionSignal(new KeyValues("FileSelected", "fullpath", fullpath));
			PostMessage(this, new KeyValues("Close"));
