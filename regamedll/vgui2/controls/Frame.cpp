...
namespace
{
	...
	public:
		GripPanel(Frame *dragFrame, const char *name, int xdir, int ydir) : Panel(dragFrame, name)
		{
			...
			m_pImage = scheme()->GetImage( "resource/icon_resizer", false );
		}
...
namespace vgui
{
	...
	public:
		...
		FrameButton(Panel *parent, const char *name, const char *text) : Button(parent, name, "")
		{
			...
			m_pImage = scheme()->GetImage( text, false );
		}
...
Frame::Frame(Panel *parent, const char *panelName, bool showTaskbarIcon) : EditablePanel(parent, panelName)
{
	...
	SetTitle("Untitled", parent ? false : true);
...
{
	static char buf[512];
	_snprintf(buf, sizeof(buf), "%s, string title", BaseClass::GetDescription());
	return buf;
}
...
void Frame::OnCommand(const char *command)
{
	...
	else if (!_stricmp(command, "CloseModal"))
	{
		CloseModal();
	}
