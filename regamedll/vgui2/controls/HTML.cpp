//========= Copyright Valve 2013 Corporation, All rights reserved. ============//
//
// Purpose: 
...
HTML::HTML(Panel *parent, const char *name, bool allowJavaScript, bool bPopupWindow) : Panel(parent, name)
{
	...
	{
		m_Serializer = surface()->AccessChromeHTMLController()->CreateSerializer(this);
		surface()->AccessChromeHTMLController()->CreateBrowser( m_Serializer->GetResponseTarget(), bPopupWindow, "Valve Half-Life" );
	}
	...
	m_pContextMenu->SetItemVisible( m_iCopyLinkMenuItemID, false );
}
...
void HTML::Paint()
{
	...
	if ( m_iHTMLTextureID != 0 )
	{
		...
		surface()->DrawFilledRect( 0, 0, tw, tt );
...
void HTML::OnMouseReleased(MouseCode code)
{
	if ( code == MOUSE_LEFT )
	{
		...
		if ( m_sDragURL.IsValid() && input()->GetMouseOver() != GetVPanel() && input()->GetMouseOver() != NULL )
		{
...
//			surface()->StartDragDropText( m_sDragURL );
	...
	if ( m_sDragURL.IsValid() && !input()->GetCursorOveride() )
	{
...
{
	m_bInFind = false;
	if ( !Q_stricmp( m_sLastSearchString, pchSubStr ) ) // same string as last time, lets fine next
		m_bInFind = true;
...
//-----------------------------------------------------------------------------
void HTML::BrowserReady( void )
{
	if ( m_sPendingURLLoad.IsValid() )
	{
...
//-----------------------------------------------------------------------------
void HTML::BrowserNeedsPaint( int textureid, int wide, int tall, const unsigned char *rgba, int updatex, int updatey, int updatewide, int updatetall, int combobox_wide, int combobox_tall, const unsigned char *combobox_rgba )
{
	int tw = 0, tt = 0;
	if ( m_iHTMLTextureID != 0 )
	{
	...
	if ( m_iHTMLTextureID == 0 || tw != wide || tt != tall )
	{
		if ( m_iHTMLTextureID != 0 )
		...
		surface()->DrawSetTextureBGRA( m_iHTMLTextureID, rgba, wide, tall );// BR FIXME - this call seems to shift by some number of pixels? (old DrawSetTextureRGBAEx
	...
	if ( m_pComboBoxHost->IsVisible() )
	{
		...
		if ( m_iComboBoxTextureID == 0  || tw != combobox_wide || tt != combobox_tall )
		{
			...
			surface()->DrawSetTextureBGRA( m_iComboBoxTextureID, combobox_rgba, combobox_wide, combobox_tall );
...
void HTML::BrowserPopupHTMLWindow( const char *url, int wide, int tall, int x, int y )
{
	...
	p->SetBounds( x, y, wide + 80, tall + 12  );
...
//-----------------------------------------------------------------------------
void HTML::BrowserLoadingResource( void )
{
	UpdateCachedHTMLValues();
}
...
//-----------------------------------------------------------------------------
void HTML::BrowserHideToolTip( void )
{
...
void HTML::BrowserLinkAtPositionResponse( const char *url, int x, int y )
{
	...
	m_pContextMenu->SetItemVisible( m_iCopyLinkMenuItemID, m_LinkAtPos.m_sURL.IsValid() );
	...
	{
		m_bRequestingCopyLink = false;
		if ( !m_LinkAtPos.m_sURL.IsValid() )
			system()->SetClipboardText( m_LinkAtPos.m_sURL, Q_strlen( m_LinkAtPos.m_sURL ) );
		else
