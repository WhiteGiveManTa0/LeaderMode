...
Menu::Menu(Panel *parent, const char *panelName) : Panel(parent, panelName)
{
	...
	if( IsProportional() )
	{
		...
		m_iProportionalScrollBarSize = static_cast<int>( static_cast<float>( SCROLLBAR_SIZE )*( static_cast<float>( sw )/ static_cast<float>( width )));
		m_iMenuItemHeight =  static_cast<int>( static_cast<float>( DEFAULT_MENU_ITEM_HEIGHT )*( static_cast<float>( sw )/ static_cast<float>( width )));
	}
	else
	{
		m_iProportionalScrollBarSize = SCROLLBAR_SIZE;
...
void Menu::LayoutScrollBar()
{
	...
	m_pScroller->SetPos(wide - m_iProportionalScrollBarSize, 0);
...
void Menu::SetItemEnabled(const char *itemName, bool state)
{
	FOR_EACH_LL( m_MenuItems, i )
	{
		if ((strcmp(itemName, m_MenuItems[i]->GetName())) == 0)
		{
...
void Menu::SetItemVisible(const char *itemName, bool state)
{
	FOR_EACH_LL( m_MenuItems, i )
	{
		if ((_stricmp(itemName, m_MenuItems[i]->GetName())) == 0)
		{
