...
void PropertySheet::RemoveAllPages()
{
	...
	for ( int i = c - 1; i >= 0 ; --i )
	{
		...
		if ( _showTabs )
		{
			_pageTabs[i]->RemoveActionSignalTarget( this );
		}

		PageTab *tab = _pageTabs[i];
		...
		_pageTabs.RemoveElementAt( i );
		tab->MarkForDeletion();
		
		_pages.RemoveElementAt( i );
		page->SetParent( (VPANEL)NULL );
