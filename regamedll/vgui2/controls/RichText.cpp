...
// Purpose: Constructor
//-----------------------------------------------------------------------------
RichText::RichText(Panel *parent, const char *panelName, bool bTrustedInput) : BaseClass(parent, panelName)
{
	_font = INVALID_FONT;

	m_bTrustInput = bTrustedInput;
...
void RichText::SetFgColor( Color color )
{
	...
	if ( m_FormatStream.Size() == 1 && m_FormatStream[0].color == _defaultTextColor )
	{
...
//-----------------------------------------------------------------------------
void RichText::OnSizeChanged(int newWide, int newTall)
{
...
//-----------------------------------------------------------------------------
int RichText::DrawChar(wchar_t ch, TRenderState &renderState, HFont font, int index)
{
	// add to the current position
	if (iswprint(ch))
	{
		int charWide = surface()->GetCharacterWidth(font, ch);
...
void RichText::PaintBackground()
{
	...
	for (int j = 0; j < _clickableTextPanels.GetCount(); j++)
	{
	...
	// draw the text
	for (int i = startIndex; i < m_TextStream.Count() && renderState.y < tall; i++)
	{
		wchar_t ch = m_TextStream[i];
		...
		renderState.x += DrawChar(ch, renderState, _font, i);
	}
}
...
int RichText::GetClickableTextIndexStart(int startIndex)
{
	...
	for (int i = 0; i < _clickableTextPanels.GetCount(); i++)
	{
...
//-----------------------------------------------------------------------------
void RichText::InsertClickableTextStart()
{
	// see if indent change already exists in text stream
	TFormatStream &prevItem = m_FormatStream[m_FormatStream.Count() - 1];
		...
		if (UpdateRenderState(i, renderState))
		{
			...
			if (renderState.textClickable != _currentTextClickable)
			{
				if (renderState.textClickable)
				{
					...
					if (clickableTextNum >= _clickableTextPanels.GetCount())
					{
						_clickableTextPanels.AddElement(new ClickPanel(this));
					}
...
void RichText::LayoutVerticalScrollBarSlider()
{
	...
	_vertScrollBar->SetPos(wide - _scrollBarSize, itop);
...
void RichText::CopySelected()
{
	...
	if (GetSelectedRange(x0, x1))
	{
		...
		buf.AddToTail('\0');
		system()->SetClipboardText(buf.Base(), x1 + iExtraChars - x0);
	}
...
void RichText::ApplySettings(KeyValues *inResourceData)
{
	...
	{
		delete [] m_pszInitialText;
		m_pszInitialText = new char[strlen(text) + 1];
		strcpy(m_pszInitialText, text);
		SetText(text);
	}
}
...
const char *RichText::GetDescription()
{
	...
	_snprintf(buf, sizeof(buf), "%s, string text", BaseClass::GetDescription());
	return buf;
}
...
int RichText::ParseTextStringForUrls(const char *text, int startPos, char *resultBuffer, int resultBufferSize, bool &clickable)
{
	...
	while (text[i] != 0)
	{
		...
		if (m_bTrustInput && !_strnicmp(text + i, "www.", 4))
		{
