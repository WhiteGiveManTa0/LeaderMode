...
void TextEntry::OnKeyTyped(wchar_t unichar)
{
	...
	if (!iswprint(unichar) || unichar == 9 ) // tab key (code 9) is printable but handled elsewhere
		return;
...
void TextEntry::GetText(wchar_t *wbuf, int bufLen)
{
	...
	if (m_TextStream.Count())
	{
		int terminator = min(len, bufLen - 1);
...
const char *TextEntry::GetDescription()
{
	...
	_snprintf(buf, sizeof(buf), "%s, bool textHidden, bool editable, bool unicode, bool NumericInputOnly, int maxchars", BaseClass::GetDescription());
	return buf;
}
